function  [yb,solb]=solvebeceq(pp,np,gr,bbr,ci,na)
    x = gr.x;
    l = min(np.maxx,pp.becrad);
    id = find(x<l);
    xred = [x(id) l];
    
    idensity = getidensity(xred);
    pol = interp1(xred,idensity,'linear','pp');
...
%     solv.parameters
    ...
    function ybprime = bvpfun(x,yb,mub)
        ybprime=[yb(2); 2*(pp.etaab*na*pp.beta*ppval(pol,x)- mub)*yb(1) + 2*pp.beta*yb(1)^3];
    end

    function ybprime = bvpfunvortex(x,yb,mub)
        ybprime=[yb(2); 2*(pp.etaab*na*pp.beta*ppval(pol,x)- mub)*yb(1) + 2*pp.beta*(x^2)*yb(1)^3];
    end
    ...
    function di = getidensity(x)
        yi = 0;
        for n = 1:np.nbr
            yi = yi + ci(n)*besselj(0,x*bbr.z(n)/np.maxx)/bbr.Jnorm(n);
        end
        di = yi.^2;
    end
end
